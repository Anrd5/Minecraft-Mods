...
# - Session Mode: tmux-based; after Setup completes, auto-detach to main Termux prompt (when explicitly invoked)
#
# Usage:
...
VERSION="2025-08-23-ui13-fix-menu-monitor"
...
BOOT_SCRIPT_VERSION="4"  # boot runs local script directly (no curl)
...
# Strip ANSI escape sequences robustly (no errors/noise)
ansi_strip() {
  printf "%s" "${1-}" | sed -E 's/\x1B```math
[0-9;?]*[ -/]*[@-~]//g; s/\x1B[@-Z\\-_]//g' 2>/dev/null || printf "%s" "${1-}"
}

# Fixed pad_to: uses ANSI-stripped width for padding; truncates without leaks.
pad_to() {
...
# --------- Selection UI ---------
# setup_reinstall [detach_after_setup (1|0)]
setup_reinstall() {
  local detach_after="${1:-1}"
  ...
  # Session Mode: auto-detach to main Termux after setup only if explicitly invoked
...
start_monitor_from_menu() {
  ...
  # Safety: if nothing selected yet, jump to Setup first, then continue
  IFS=' ' read -r -a selected_builtin <<< "$(read_config)"
  mapfile -t selected_custom < <(custom_apks_selected_lines || true)
  if [ ${#selected_builtin[@]} -eq 0 ] && [ ${#selected_custom[@]} -eq 0 ]; then
    printf "%sNo APKs selected. Opening Setup first...%s\n\n" "$DIM" "$RESET"
    sleep 1
    setup_reinstall 0   # do NOT auto-detach when invoked from here
    # Re-check after setup
    IFS=' ' read -r -a selected_builtin <<< "$(read_config)"
    mapfile -t selected_custom < <(custom_apks_selected_lines || true)
    if [ ${#selected_builtin[@]} -eq 0 ] && [ ${#selected_custom[@]} -eq 0 ]; then
      printf "%sStill no APKs selected. Aborting monitor.%s\n\n" "$DIM" "$RESET"
...
misc_menu() {
  while true; do
    ...
    printf " - After Setup completes, client detaches back to Termux prompt (when invoked directly)\n\n"
    printf "%s────────────────────────────────────────%s\n\n" "$GRAY" "$RESET"
...
apk_menu() {
  while true; do
    ...
    printf "  %s1)%s Setup APK selection\n" "$ORANGE" "$RESET"
    printf "  %s2)%s Continuous monitoring (download + install)\n" "$ORANGE" "$RESET"
    printf "  %s3)%s Manage custom APKs\n" "$ORANGE" "$RESET"
    printf "  %s4)%s Log Management\n" "$ORANGE" "$RESET"
    printf "  %s5)%s Miscellaneous\n" "$ORANGE" "$RESET"
    printf "  %sq)%s Back to main menu\n\n" "$ORANGE" "$RESET"
    printf "%s────────────────────────────────────────%s\n\n" "$GRAY" "$RESET"
    read -r -p "Choice: " c
    case "$c" in
      1) setup_reinstall 1 ;;
      2) start_monitor_from_menu ;;
      3) manage_custom_apks_menu ;;
      4) logs_menu ;;
      5) misc_menu ;;
      q|Q) break ;;
      *) echo "Invalid."; read -r -p "Enter..." _ ;;
...
case "${1:-}" in
  ...
  --setup)      setup_reinstall 1; exit 0 ;;
  ...
  --help)
    ...
    echo "  $0 --setup              # select built-in APKs (auto-detach in Session Mode when invoked directly)"
