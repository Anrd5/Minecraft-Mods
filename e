...
# - Misc: tmux dashboard, watchdog, persisted monitor settings
#
# Usage:
...
VERSION="2025-08-30-ui8-table+clean-menu-v2-patch2-tmuxdash"
...
persist_get() {
  local key="$1"
  [ -f "$PERSIST_ENV" ] || { echo ""; return 0; }
  grep -E "^${key}=" "$PERSIST_ENV" 2>/dev/null | head -n1 | cut -d= -f2- || echo ""
}
persist_set() {
  local key="$1" value="$2"
  mkdir -p "$APP_DIR" || true
  [ -f "$PERSIST_ENV" ] || : > "$PERSIST_ENV"
  if grep -qE "^${key}=" "$PERSIST_ENV" 2>/dev/null; then
    sed -i "s|^${key}=.*|${key}=${value}|" "$PERSIST_ENV" 2>/dev/null || {
      local tmp; tmp=$(mktemp 2>/dev/null || echo "$PERSIST_ENV.tmp")
      sed "s|^${key}=.*|${key}=${value}|" "$PERSIST_ENV" > "$tmp" && mv "$tmp" "$PERSIST_ENV"
    }
  else
    printf "%s=%s\n" "$key" "$value" >> "$PERSIST_ENV"
  fi
}
persist_ensure_defaults() {
  local sess interval action script_path
  sess="$(persist_get TMUX_SESSION)"; [ -n "$sess" ] || sess="reinstall"
  interval="$(persist_get INTERVAL)"; [ -n "$interval" ] || interval="$MONITOR_INTERVAL_DEFAULT"
  action="$(persist_get ACTION)";    [ -n "$action" ] || action="install"
  script_path="$(persist_get SCRIPT)"; [ -n "$script_path" ] || script_path="$(script_self_path)"
  persist_set TMUX_SESSION "$sess"
  persist_set INTERVAL "$interval"
  persist_set ACTION "$action"
  persist_set SCRIPT "$script_path"
  if [ -z "$(persist_get USE_TSU)" ]; then
    persist_set USE_TSU "$([ "$RUNNING_AS_ROOT" -eq 1 ] && echo 0 || (command -v tsu >/dev/null 2>&1 && echo 1 || echo 0))"
  fi
}
...
if command -v tmux >/dev/null 2>&1; then
  ...
  if ! tmux has-session -t "$TMUX_SESSION" >/dev/null 2>&1; then
    ...
    if command -v tsu >/dev/null 2>&1 && [ "${USE_TSU:-0}" -eq 1 ]; then
      tmux new-session -d -s "$TMUX_SESSION" "while :; do tsu -c $CMD; echo 'restarting in 10s'; sleep 10; done" >>"$LOG" 2>&1 || true
    else
    ...
    if ! pgrep -f "reinstall.sh.*--monitor" >/dev/null 2>&1; then
      ...
      if command -v tsu >/dev/null 2>&1 && [ "${USE_TSU:-0}" -eq 1 ]; then
        tmux new-window -t "$TMUX_SESSION" "while :; do tsu -c $CMD; echo 'restarting in 10s'; sleep 10; done" >>"$LOG" 2>&1 || true
      else
  ...
  if ! pgrep -f "reinstall.sh.*--monitor" >/dev/null 2>&1; then
    ...
    if command -v tsu >/dev/null 2>&1 && [ "${USE_TSU:-0}" -eq 1 ]; then
      nohup tsu -c bash -lc "$CMD" >>"$LOG" 2>&1 &
    else
...
maintenance_run_now() {
  persist_ensure_defaults
  generate_watchdog
  bash "$WATCHDOG_SH" >>"$WATCHDOG_LOG" 2>&1 || true
}
maintenance_restart_session() {
  persist_ensure_defaults
  local sess; sess="$(persist_get TMUX_SESSION)"; [ -n "$sess" ] || sess="reinstall"
  if command -v tmux >/dev/null 2>&1; then
...
ui_clear()   { if [ "$FANCY_UI" -eq 1 ] && command -v tput >/dev/null 2>&1; then tput clear 2>/dev/null || printf "\033c"; else printf "\033[2J\033[H"; fi; }
repeat_char() { local ch="$1" n="$2"; printf "%s" "$(printf "%0.s$ch" $(seq 1 "$n"))"; }

# ANSI stripper
ansi_strip() { printf "%s" "${1-}" | sed -E 's/\x1B```math
...
if [ -f "\$PERSIST_ENV" ]; then . "\$PERSIST_ENV"; fi
...
if command -v tmux >/dev/null 2>&1; then
  ...
  if ! tmux has-session -t "\$TMUX_SESSION" >/dev/null 2>&1; then
    ...
    if command -v tsu >/dev/null 2>&1 && [ "\${USE_TSU:-0}" -eq 1 ]; then
      tmux new-session -d -s "\$TMUX_SESSION" "while :; do tsu -c \$CMD; echo 'restarting in 10s'; sleep 10; done" >>"\$BOOT_LOG" 2>&1 || true
    else
      tmux new-session -d -s "\$TMUX_SESSION" "while :; do eval \$CMD; echo 'restarting in 10s'; sleep 10; done" >>"\$BOOT_LOG" 2>&1 || true
    fi
  else
    echo "Boot: tmux session '\$TMUX_SESSION' already exists" >> "\$BOOT_LOG"
  fi
else
  echo "Boot: tmux not available, starting detached monitor" >> "\$BOOT_LOG"
  if command -v tsu >/dev/null 2>&1 && [ "\${USE_TSU:-0}" -eq 1 ]; then
    nohup tsu -c bash -lc "\$CMD" >> "\$BOOT_LOG" 2>&1 &
  else
...
# --------- Miscellaneous → Tmux Dashboard ---------
tmux_dashboard() {
  while true; do
    banner_section "TMUX SESSION DASHBOARD"
    local tmux_ok=0 tmux_ver="" sess_lines="" server_state="unknown"
    if command -v tmux >/dev/null 2>&1; then tmux_ok=1; tmux_ver="$(tmux -V 2>/dev/null || echo "")"; fi

    persist_ensure_defaults
    local psess pint paction pscript putsu
    psess="$(persist_get TMUX_SESSION)"; [ -n "$psess" ] || psess="reinstall"
    pint="$(persist_get INTERVAL)"; [ -n "$pint" ] || pint="$MONITOR_INTERVAL_DEFAULT"
    paction="$(persist_get ACTION)"; [ -n "$paction" ] || paction="install"
    pscript="$(persist_get SCRIPT)"; [ -n "$pscript" ] || pscript="$(script_self_path)"
    putsu="$(persist_get USE_TSU)"; [ -n "$putsu" ] || putsu="0"

    local mon_alive="no"
    if [ -f "$MONITOR_PID" ]; then
      local mpid; mpid="$(cat "$MONITOR_PID" 2>/dev/null || echo "")"
      if [ -n "$mpid" ] && kill -0 "$mpid" 2>/dev/null; then mon_alive="yes (pid $mpid)"; fi
    fi

    printf "%sTmux installed:%s %s\n" "$GRAY" "$RESET" "$([ "$tmux_ok" -eq 1 ] && echo "yes ($tmux_ver)" || echo "no")"
    if [ "$tmux_ok" -eq 1 ]; then
      if tmux ls >/dev/null 2>&1; then
        server_state="running"
        echo
        printf "%sActive sessions:%s\n" "$BOLD" "$RESET"
        if tmux ls -F '#S\t#{session_windows}\t#{session_attached}\t#{session_created_string}' >/dev/null 2>&1; then
          tmux ls -F '#S\t#{session_windows}\t#{session_attached}\t#{session_created_string}' 2>/dev/null | while IFS=$'\t' read -r sname wins attached created; do
            printf "  - %-18s windows=%-2s attached=%-1s created=%s\n" "$sname" "$wins" "$attached" "$created"
          done
        else
          tmux ls 2>/dev/null | sed 's/^/  - /'
        fi
      else
        server_state="no server (no sessions)"
        printf "%sActive sessions:%s none (tmux server not running)\n" "$BOLD" "$RESET"
      fi
    fi

    echo
    printf "%sPersisted monitor settings:%s\n" "$BOLD" "$RESET"
    printf "  - Session:     %s\n" "$psess"
    printf "  - Interval:    %ss\n" "$pint"
    printf "  - Mode:        %s\n" "$([ "$paction" = "download" ] && echo "download-only" || echo "install")"
    printf "  - Script:      %s\n" "$pscript"
    printf "  - Use tsu:     %s\n" "$([ "$putsu" = "1" ] && echo "yes" || echo "no")"
    printf "  - Monitor PID: %s\n" "$mon_alive"
    printf "  - Boot script: %s\n" "$([ -f "$BOOT_SCRIPT" ] && echo "present" || echo "missing")"
    printf "  - Watchdog:    %s\n" "$([ -x "$WATCHDOG_SH" ] && echo "present" || echo "missing")"
    printf "  - JobScheduler:%s\n" "$([ "$(command -v termux-job-scheduler >/dev/null 2>&1; echo $?)" -eq 0 ] && echo "available" || echo "unavailable")"

    echo
    printf "%sOptions:%s\n" "$BOLD" "$RESET"
    printf "  %s1)%s Run 24/7 tmux maintenance now\n" "$ORANGE" "$RESET"
    printf "  %s2)%s Attach to session [%s]\n" "$ORANGE" "$RESET" "$psess"
    printf "  %s3)%s Force recreate session [%s]\n" "$ORANGE" "$RESET" "$psess"
    printf "  %s4)%s Toggle monitor mode (install/download-only)\n" "$ORANGE" "$RESET"
    printf "  %s5)%s Set monitor interval (seconds)\n" "$ORANGE" "$RESET"
    printf "  %s6)%s View logs (reinstall/boot/watchdog)\n" "$ORANGE" "$RESET"
    printf "  %sQ)%s Back\n\n" "$ORANGE" "$RESET"
    read -r -p "Choice: " tchoice

    case "${tchoice:-}" in
      1)
        echo "Running maintenance..."
        maintenance_run_now
        echo "Done. Check: $WATCHDOG_LOG"
        read -r -p "Enter..." _
        ;;
      2)
        if [ "$tmux_ok" -ne 1 ]; then echo "tmux is not installed."; read -r -p "Enter..." _; continue; fi
        read -r -p "Session name [$psess]: " asn; asn="${asn:-$psess}"
        if tmux has-session -t "$asn" 2>/dev/null; then
          ui_alt_off
          tmux attach -t "$asn"
          ui_alt_on
        else
          echo "Session '$asn' does not exist. Use option 1 or 3 to (re)create."
          read -r -p "Enter..." _
        fi
        ;;
      3)
        if [ "$tmux_ok" -ne 1 ]; then echo "tmux is not installed."; read -r -p "Enter..." _; continue; fi
        read -r -p "Recreate persisted session '$psess'? (y/N): " yn
        [[ "$yn" =~ ^[Yy]$ ]] || continue
        maintenance_restart_session
        echo "Recreated. Check with: tmux ls"
        read -r -p "Enter..." _
        ;;
      4)
        local newmode="install"
        if [ "$paction" = "install" ]; then newmode="download"; fi
        persist_set ACTION "$newmode"
        echo "Mode set to: $([ "$newmode" = "download" ] && echo "download-only" || echo "install")"
        if command -v tmux >/dev/null 2>&1; then
          echo "Restarting session to apply..."
          maintenance_restart_session
        fi
        read -r -p "Enter..." _
        ;;
      5)
        read -r -p "New interval seconds (current $pint): " ns
        if [[ "$ns" =~ ^[0-9]+$ ]] && [ "$ns" -ge 30 ] && [ "$ns" -le 86400 ]; then
          persist_set INTERVAL "$ns"
          echo "Interval updated to $ns. Restarting session to apply..."
          maintenance_restart_session
        else
          echo "Invalid. Use 30..86400."
        fi
        read -r -p "Enter..." _
        ;;
      6)
        echo "Logs:"
        echo "  1) reinstall.log"
        echo "  2) boot.log"
        echo "  3) watchdog.log"
        read -r -p "Pick: " lg
        case "$lg" in
        ...
        if [ -n "$f" ] && [ -f "$f" ]; then
          if command -v less >/dev/null 2>&1; then less "$f"; else tail -n 200 "$f"; read -r -p "Enter..." _; fi
        else
          echo "No log file: $f"; read -r -p "Enter..." _
        fi
        ;;
      Q|q) break ;;
...
misc_menu() {
  while true; do
    ...
    printf "%s%sTools:%s\n\n" "$BOLD" "$WHITE" "$RESET"
    printf "  %s1)%s Tmux Session Dashboard\n" "$ORANGE" "$RESET"
    printf "  %sQ)%s Back\n\n" "$ORANGE" "$RESET"
    read -r -p "Choice: " c
    case "$c" in 1|"") tmux_dashboard ;; Q|q) break ;; *) echo "Invalid."; read -r -p "Enter..." _ ;; esac
  done
}
...
apk_menu() {
  while true; do
    ...
    printf "%s────────────────────────────────────────%s\n\n" "$GRAY" "$RESET"
    read -r -p "Choice: " c
    case "$c" in 1|"") start_monitor_from_menu ;; 2) setup_reinstall ;; 3) force_prompt_and_run ;; 4) manage_custom_apks_menu ;; q|Q) break ;; *) echo "Invalid."; read -r -p "Enter..." _ ;; esac
  done
}
